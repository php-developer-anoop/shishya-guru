  ...
  <div class="content">
    <div class="container-fluid">
      ...
      <div class="card-header">
          <a href="<?= base_url(ADMINPATH . 'add-cms') ?>" class="btn btn-success m-auto float-right">Add CMS Page</a>
        </div>
        <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'cms-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'cms-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
...
function action_render(data, type, row, meta) {
    let output = '';
    if (type === 'display') {
        output = '<a href="<?= base_url(ADMINPATH . "add-cms?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white " title="Edit CMS Page"><i class="fa fa-edit"></i></a> ';
    }
    return output;
}
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_cms_list')`;
