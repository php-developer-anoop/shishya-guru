    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-header">
          <a href="<?= base_url(ADMINPATH . 'add-banner') ?>" class="btn btn-success m-auto float-right">Add Banner</a>
        </div>
        <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'banner-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'banner-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
            ...
            { data: "id", "title": "Banner Details","render":banner_detail },
...
var banner_detail = ( data, type, row, meta )=>{
  var data = '';
  ...
  if(type === 'display'){
        ...
        data += '<span class="fotr_10"><b>Slug : </b>'+slug+'</span>' ;
...
function action_render(data, type, row, meta) {
    let output = '';
    if (type === 'display') {
        output = '<a href="<?= base_url(ADMINPATH . "add-banner?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white " title="Edit Banner"><i class="fa fa-edit"></i></a> ';
    }
    return output;
}
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_banner_list')`;
