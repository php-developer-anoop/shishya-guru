  ...
  <div class="content">
    <div class="container-fluid">
      ...
      <div class="card-header">
          <a href="<?= base_url(ADMINPATH . 'add-blog') ?>" class="btn btn-success m-auto float-right">Add Blog</a>
        </div>
        <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'blog-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'blog-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
            ...
            { data: "id", "title": "Blog Details","render":blog_detail },
...
var blog_detail = ( data, type, row, meta )=>{
  var data = '';
  let slug= row.slug!=null?row.slug:"";
  let page_url= row.page_url!=null?row.page_url:"";
  let blog_category_name= row.blog_category_name!=null?row.blog_category_name:"";
  let blog_title= row.blog_title!=null?row.blog_title:"";
  if(type === 'display'){
        data += '<span class="fotr_10"><b>Category : </b>'+blog_category_name+'</span><br>' ;
        data += '<span class="fotr_10"><b>Title : </b>'+blog_title+'</span><br>' ;
        data += '<span class="fotr_10"><b>Slug : </b><a href="'+page_url+'">'+slug+'</span>' ;
  }
return data;
}
...
function action_render(data, type, row, meta) {
    ...
    if (type === 'display') {
        ...
        output= '<a href="<?= base_url(ADMINPATH . "add-blog?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white " title="Edit Blog"><i class="fa fa-edit"></i></a> ';
    }
    return output;
}
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_blogs_list')`;
