...
class Qualification extends BaseController {
    ...
    public function __construct() {
        ...
        $this->table = 'dt_qualification_list';
    }
    public function index() {
        $data['menu'] = 'Qualification Master';
        $data['title'] = 'Qualification List';
        adminView('qualification-list', $data);
    }
    function add_qualification() {
        $id = !empty($this->request->getVar('id')) ? $this->request->getVar('id') : '';
        $data = [];
        $data["menu"] = "Qualification Master";
        $data["title"] = !empty($id)?"Edit Qualification":"Add Qualification";
        $savedData = $this->c_model->getSingle($this->table, '*', ['id' => $id]);
        $data['id'] = !empty($savedData['id']) ? $savedData['id'] : $id;
        $data['qualification_name'] = !empty($savedData['qualification_name']) ? $savedData['qualification_name'] : '';
        $data['status'] = !empty($savedData['status']) ? $savedData['status'] : 'Active';
        adminView('add-qualification', $data);
    }
    public function save_qualification() {
        ...
        $data['qualification_name'] = ucwords(trim($post['qualification_name']));
        ...
        $url = base_url(ADMINPATH . 'add-qualification') . '?id=' . $last_id;
    ...
    public function getRecords() {
        ...
        if (!empty($get["search"]["value"])) {
            ...
            $where[" qualification_name LIKE '%" . $searchString . "%'"] = null;
            $limit = 100;
            $start = 0;
        }
