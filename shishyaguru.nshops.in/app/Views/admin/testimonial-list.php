    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-header d-none">
          <a href="<?= base_url(ADMINPATH . 'add-testimonial') ?>" class="btn btn-success m-auto float-right">Add Testimonial</a>
        </div>
        <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'testimonial-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'testimonial-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
            ...
            { data: "id", "title": "Testimonial Detail","render":testimonial_detail },
...
var testimonial_detail = ( data, type, row, meta )=>{
  var data = '';
  let name = row.name!=null?row.name:"";
  let location = row.location!=null?row.location:"";
  let rating = row.rating!=null?row.rating:"";
  let tutor = row.tutor!=null?row.tutor:"N/A";

  if(type === 'display'){
        data += '<span class="fotr_10"><b>Name : </b>'+name+'</span><br>' ;
        data += '<span class="fotr_10"><b>Location : </b>'+location+'</span><br>' ;
        data += '<span class="fotr_10"><b>Rating : </b>'+rating+'</span><br>' ;
        data += '<span class="fotr_10"><b>Tutor : </b>'+tutor+'</span>' ;
...
function action_render(data, type, row, meta) {
    let output = '';
    if (type === 'display') {
        output = '<a href="<?= base_url(ADMINPATH . "add-testimonial?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white " title="Edit Testimonial"><i class="fa fa-edit"></i></a> ';
    }
    return output;
}
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_testimonial_list')`;
