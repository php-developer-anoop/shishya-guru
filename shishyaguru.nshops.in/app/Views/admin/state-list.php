    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-header">
          <a href="<?= base_url(ADMINPATH . 'add-state') ?>" class="btn btn-success m-auto float-right">Add State</a>
        </div>
        <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'state-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'state-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
...
function action_render(data, type, row, meta) {
    let output = '';
    if (type === 'display') {
        output = '<a href="<?= base_url(ADMINPATH . "add-state?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white " title="Edit State"><i class="fa fa-edit"></i></a> ';
    }
    return output;
}

function add_city_render(data, type, row, meta) {
    let output = '';
    if (type === 'display') {
        output= '<a href="<?= base_url(ADMINPATH . "add-city?state_id=") ?>' + row.id + '" class="btn btn-primary btn-sm text-white " title="Add City"><i class="fa fa-plus"></i></a> ';
    }
    return output;
}
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_state_list')`;
