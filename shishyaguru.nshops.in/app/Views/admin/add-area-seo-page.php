    ...
    <div class="container-fluid">
      <div class="card card-default">
        <div class="card-header">
          <a href="<?= base_url('admin/area-seo-page-list') ?>" class="btn btn-success m-auto"
            style="float:right;position:relative;">
          Area Seo Pages List
          </a>
        </div>
        <div class="card-header">
          <?= form_open_multipart(base_url(ADMINPATH.'save-area-seo-page')); ?>
          ...
          <input type="hidden" name="seo_page_id" value="<?= $seo_page_id ?>">
          <input type="hidden" name="state_id" value="<?= $state_id ?>">
          <input type="hidden" name="city_id" value="<?= $city_id ?>">
          <div class="form-group row">
          <div class="col-sm-4" >
              <?= form_label('Select Area <span class="text-danger">*</span>', 'area_id', ['class' => 'col-form-label']) ?>
              <select name="area_id"  required class="form-control select2">
                <option value="">Select Area</option>
                <?php if(!empty($area_list)){foreach($area_list as $bkey=>$bvalue){?>
                <option value="<?=$bvalue['id']?>" <?=!empty($area_id) && ($area_id==$bvalue['id'])?'selected':''?>><?=$bvalue['area_name']?></option>
                <?php }} ?>
              </select>
            </div>
            </div>
