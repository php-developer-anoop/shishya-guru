    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-header">
          <a href="<?= base_url(ADMINPATH . 'add-lead') ?>" class="btn btn-success m-auto float-right">Add Lead</a>
        </div>
        <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'lead-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'lead-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
            ...
            { data: "id", "title": "Lead Details","render":lead_detail },
...
var lead_detail = ( data, type, row, meta )=>{
  ...
  let cost_per_lead = row.cost_per_lead!=null?row.cost_per_lead:"";

  if(type === 'display'){
        data += '<span class="fotr_10"><b>Cost Per Lead : </b>'+cost_per_lead+'</span><br>' ;
        data += '<span class="fotr_10"><b>Class Group : </b>'+class_group_name+'</span><br>' ;
...
function action_render(data, type, row, meta) {
    let output = '';
    if (type === 'display') {
        output = '<a href="<?= base_url(ADMINPATH . "add-lead?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white " title="Edit Lead"><i class="fa fa-edit"></i></a> ';
    }
    return output;
}
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_leads_list')`;
