    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-header">
          <a href="<?= base_url(ADMINPATH . 'add-board') ?>" class="btn btn-success m-auto float-right">Add Board</a>
        </div>
        <div class="card-body">
...
<script>
    function getTotalRecordsData(qparam) {
        $.ajax({
            url: '<?= base_url(ADMINPATH . 'board-data'); ?>?' + qparam,
            type: "POST",
    ...
    function loadAllRecordsData(qparam) {
        ...
        $('#responseData').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": '<?= base_url(ADMINPATH . 'board-data'); ?>' + newQueryParam,
                "type": 'POST',
                dataSrc: (res) => {
                    return res.data
                }
            ...
            { data: "board_name", "title": "Board Name" },
...
function action_render(data, type, row, meta) {
    let output = '';
    if (type === 'display') {
        output = '<a href="<?= base_url(ADMINPATH . "add-board?id=") ?>' + row.id + '" class="btn btn-success btn-sm text-white " title="Edit Board"><i class="fa fa-edit"></i></a> ';
...
function add_seo_render(data, type, row, meta) {
    ...
    if (type === 'display') {
        ...
        output = '<a href="<?= base_url(ADMINPATH . "add-seo-template?table_id=") ?>' + row.id + '&table_name=dt_boards_list" class="btn btn-primary btn-sm text-white " title="Add/Edit Seo Page"><i class="fa fa-'+icon_class+'"></i></a> ';
   ...
   function status_render(data, type, row, meta) {
    if (type === 'display') {
        ...
        const onchange = `change_status(${row.id}, 'dt_boards_list')`;
